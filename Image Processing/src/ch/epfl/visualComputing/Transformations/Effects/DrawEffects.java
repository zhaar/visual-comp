...
public class DrawEffects {
    ...
    public static EffectFunction<HoughTransformation.HoughData> drawLines(PApplet ctx, int left,  int width, int minVotes) {
        return new EffectFunction<>(hough -> {
            MyList.zipWithIndex(hough.acc).stream().filter(p -> p._1() > minVotes).forEach(p -> {
                int idx = p._2();
                int rDim = hough.rDim;
...
//                float discretizationStepsPhi = hough.phiStep;
                ...
                float r = (accR - rDim * 0.5f) * hough.rStep;
